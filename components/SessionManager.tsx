  ...
  const validateDriveUrl = (url: string) => {
    if (formData.isDriveLink && url.trim() !== '') {
       ...
       if (!googleUrlPattern.test(url)) {
         setUrlError('Por favor, introduce una URL válida de Google Drive o Docs');
         return false;
       }
    }
    setUrlError(null);
  ...
  return (
    <>
      <div className="flex flex-col md:flex-row gap-6 h-[calc(100vh-140px)]">
        ...
        <div className="w-full md:w-1/3 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col no-print">
          <div className="p-4 bg-earth-50 border-b border-gray-200 flex justify-between items-center">
            <h3 className="font-bold text-gray-700">Sesiones Clínicas</h3>
            {canEdit && (
              <button 
          ...
          <div ref={sidebarRef} className="overflow-y-auto flex-1 p-2 space-y-2">
            {clinicalSessions.length > 0 ? (
              ...
              <div className="p-8 text-center text-gray-400 text-sm">No hay sesiones clínicas.</div>
            )}
          </div>
        </div>
        ...
        <div className="w-full md:w-2/3 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col print:border-0 print:shadow-none print:w-full">
          {selectedSession ? (
            <div className="flex-1 overflow-y-auto bg-gray-50/30 print:bg-white print:overflow-visible">
              ...
              <div className="print-only mb-10 border-b-2 border-gray-900 pb-6">
                <div className="flex justify-between items-end">
                  <div>
                    <h1 className="text-2xl font-black uppercase tracking-tight">Acta de Sesión Clínica</h1>
                    <p className="text-lg font-bold text-gray-700">Zona Básica de Salud Forcall</p>
                    <p className="text-sm text-gray-500 mt-1 uppercase font-bold tracking-widest">Els Ports, Castellón</p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold">FECHA: {selectedSession.date.toLocaleDateString('es-ES')}</p>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1 no-print">
                       <span className={`text-xs font-bold px-2 py-0.5 rounded uppercase ${selectedSession.isCancelled ? 'bg-red-100 text-red-700' : 'text-forcall-600 bg-forcall-50'}`}>
                         {selectedSession.isCancelled ? 'Cancelada' : 'Sesión Clínica'}
                       </span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3 text-sm">
                      <div className="flex items-center gap-1.5 text-gray-600 bg-gray-100 px-2.5 py-1 rounded-lg print:bg-white print:p-0 print:font-bold">
                        ...
                        <span>Ponente: {selectedSession.speaker}</span>
                      </div>
                      <div className="flex items-center gap-1.5 text-gray-600 bg-gray-100 px-2.5 py-1 rounded-lg print:bg-white print:p-0 print:font-bold">
                        <span className="material-symbols-outlined text-[18px] no-print">calendar_today</span>
                        <span>Fecha: {selectedSession.date.toLocaleDateString('es-ES')} {selectedSession.time && ` a las ${selectedSession.time}`}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-2 no-print">
                    <button onClick={handlePrint} title="Imprimir o guardar como PDF" className="p-3 bg-white border border-gray-200 rounded-xl hover:bg-forcall-50 text-forcall-600 transition-all flex items-center gap-2 font-bold text-xs shadow-sm active:scale-95"><span className="material-symbols-outlined">print</span> PDF</button>
                    {canEdit && (
                      <>
              ...
              <div className="p-6 space-y-6">
                 ...
                 <div className="bg-white rounded-xl p-5 border border-gray-200 shadow-sm print:border-0 print:p-0 print:shadow-none">
                   <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2 print:text-lg">
                     ...
                     Resumen de la Sesión / Acta
                   </h3>
                   <textarea
                      value={summaryInput}
                      onChange={(e) => setSummaryInput(e.target.value)}
                      placeholder="Resume la sesión..."
                      rows={6}
                    ...
                    <div className="hidden print:block print-text-block">
                      {summaryInput || "No hay resumen redactado para esta sesión."}
                    </div>
                    ...
                    <div className="flex justify-between items-center mt-3 no-print">
                      ...
                      {canEdit && <button onClick={handleSaveSummary} className="px-5 py-2 bg-gray-900 text-white text-xs font-black uppercase tracking-widest rounded-xl hover:bg-black transition-all shadow-md active:scale-95">Guardar Acta</button>}
                    </div>
                 </div>
                 ...
                 <div className="bg-white rounded-xl p-5 border border-gray-200 shadow-sm print:border-0 print:p-0 print:shadow-none">
                   ...
                   <div className="space-y-3 mb-6">
                     {selectedSession.proposals && selectedSession.proposals.length > 0 ? (
                       ...
                       <div className="py-6 text-center text-gray-400 text-xs flex flex-col items-center gap-2 print:hidden">
                         ...
                         <p className="italic">No hay propuestas ni comentarios registrados.</p>
                       </div>
                     )}
                   </div>
                   ...
                   <div className="flex gap-2 bg-gray-50 p-2 rounded-2xl border border-gray-100 no-print">
                     <input 
                       ...
                       placeholder="Añadir una propuesta..."
                 ...
                 <div className="hidden print:grid grid-cols-2 gap-20 mt-20">
                   <div className="text-center">
                     <div className="border-t-2 border-black pt-2 text-xs font-black uppercase tracking-widest">Ponente de la Sesión</div>
                     <p className="text-[10px] mt-2 italic text-gray-500 font-medium">{selectedSession.speaker}</p>
                   </div>
                   <div className="text-center">
                     <div className="border-t-2 border-black pt-2 text-xs font-black uppercase tracking-widest">Coordinación ZBS Forcall</div>
                     <p className="text-[10px] mt-2 italic text-gray-500 font-medium">Sello de la Zona Básica</p>
                   </div>
                 </div>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-gray-400 p-8">
               ...
               <p className="font-bold uppercase text-[10px] tracking-widest">Selecciona una sesión clínica</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl max-w-2xl w-full overflow-hidden">
            <div className="p-8 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <h3 className="text-2xl font-black text-gray-900 uppercase tracking-tight">
                {isEditing ? 'Editar' : 'Programar'} Sesión Clínica
              </h3>
            ...
            <form onSubmit={handleSaveSession} className="p-8 space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div className="col-span-2">
                  <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Título de la Sesión</label>
                ...
                <div>
                  <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Ponente</label>
                ...
                <div>
                  <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Hora de Inicio</label>
              ...
              <button type="submit" className="w-full py-5 bg-gray-900 text-white rounded-[2rem] font-black uppercase tracking-[0.2em] shadow-2xl hover:bg-black transition-all active:scale-95 text-xs">
                {isEditing ? 'Actualizar' : 'Confirmar'} Sesión
              </button>
            </form>
          </div>
        </div>
      ...
      <ConfirmationModal 
        ...
        message="Esta acción no se puede deshacer y eliminará todos los comentarios y actas asociados."
