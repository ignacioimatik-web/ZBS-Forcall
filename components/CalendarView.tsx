...
export const CalendarView: React.FC<CalendarViewProps> = ({ meetings }) => {
  ...
  const weekDays = [t('calendarView.dayMon'), t('calendarView.dayTue'), t('calendarView.dayWed'), t('calendarView.dayThu'), t('calendarView.dayFri'), t('calendarView.daySat'), t('calendarView.daySun')];
  
  const meetingTypeLabel: Record<MeetingType, string> = {
    [MeetingType.TEAM]: t('meetingTypes.teamMeeting'),
    [MeetingType.MEDICINE]: t('meetingTypes.medicinaMeeting'),
    [MeetingType.NURSING]: t('meetingTypes.enfermeriaMeeting'),
    [MeetingType.CLINICAL]: t('meetingTypes.clinicalSession'),
    [MeetingType.WORKSHOP]: t('meetingTypes.workshop'),
    [MeetingType.OTHER]: t('meetingTypes.otros'),
  };
              ...
              <div className="mt-1 space-y-1">
                {events.map(ev => (
                  <button 
                    ...
                    title={ev.isCancelled ? `${t('calendarView.cancelledPrefix')}${ev.title}` : ev.title}
                    ...
                    <div className="flex justify-between items-start mb-4">
                        <div className="flex flex-col">
                            {selectedMeeting.isCancelled && (
                                <span className="text-[10px] font-bold text-red-600 bg-red-100 px-2 py-0.5 rounded w-fit mb-1">{t('calendarView.cancelled')}</span>
                            )}
                    ...
                    <div className="space-y-4 text-sm text-gray-600">
                        {selectedMeeting.isCancelled && selectedMeeting.cancellationReason && (
                             <div className="bg-red-50 p-3 rounded-lg border border-red-100">
                                <p className="text-red-800 text-xs font-bold mb-1">{t('calendarView.cancellationReason')}</p>
                        ...
                        <div className="flex items-center gap-3">
                             ...
                             <div>
                                <p className="text-xs text-gray-400">{t('calendarView.date')}</p>
                        ...
                        {selectedMeeting.speaker && (
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-xs text-gray-400">{t('calendarView.speaker')}</p>
